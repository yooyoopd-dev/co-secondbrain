// 한국어 AI 티 결정론적 탐지기 — im-not-ai(MIT)의 quick-rules를 TS/JS로 이식.
...
export function maskProtected(md) {
  const spans = [];
  const push = (re) => {
...
const countAll = (t, re) => [...t.matchAll(re)].length;
/** 문단 단위로 임계 이상 밀집한 경우만 센다. AI 티는 빈도가 아니라 밀집이 신호다. */
const denseInParagraph = (t, re, min) =>
