...
| `log.md` 추가 전용 | **원문 형식 그대로 채택**: `## [2026-04-02] ingest \| 제목`, grep 파싱 가능 | 개인 Vault 는 단일 사용자라 동시 append 문제 없음. CO 영역은 허브 DB의 events 테이블이 담당 |
| 위키 = git repo (버전·브랜치·협업 무료) | **개인 Vault 는 앱 내부 스냅샷**(Git 의존성 없음), **CO 영역은 허브 DB가 버전 관리** | 사용자가 Git 비의존을 요구. 대신 되돌리기·이력·충돌 감지는 다른 수단으로 반드시 제공 |
